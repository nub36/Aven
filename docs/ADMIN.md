...
> **Последнее обновление:** 2026-09-26 — «минимум админки» из [MVP_SCOPE.md](MVP_SCOPE.md) §4.1 п.11 представлен
> в **прототипе** (`prototype/#/admin`: пользователи, роли, аудит, миграции, health, бэкапы, feature flags,
> аварийные переключатели). Это UX-макет без backend: опасные действия требуют подтверждения и пишутся в
> демо-аудит, состав разделов соответствует §2, 3, 5, 14, 16–18, 21.
...
- **Разделение `/settings` и `/admin`** ([ADR-012](DECISIONS.md), [PROJECT_PLAN.md](PROJECT_PLAN.md), раздел 13): `/settings` — настройки конкретного пользователя; `/admin` — управление системой Aven. **Администратор не должен автоматически получать возможность действовать от имени пользователя.** Если в будущем появится support impersonation: проектирование отдельно (отдельный ADR), строгие permissions, явная заметность, журналирование, причина входа, по возможности ограничение доступа к чувствительным данным.
...
5. Support impersonation — нужен ли и в каком виде; при появлении — отдельный ADR (строгие permissions, явная заметность, журналирование, причина входа, ограничение чувствительных данных).
